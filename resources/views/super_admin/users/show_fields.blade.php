            ...
            <div class="col-xxl-5 col-12">
                <div class="d-sm-flex align-items-center mb-5 mb-xxl-0 text-center text-sm-start">
                    ...
                    <div class="ms-0 ms-sm-10 mt-5 mt-sm-0">
                        <span class="badge bg-light-{{  $users['hospital']->status === 1 ? 'success' : 'danger' }} mb-2 active-button">{{ ($users['hospital']->status === 1) ? __('messages.common.active') : __('messages.common.de_active') }}</span>
                        <h2><a href="#" class="text-decoration-none table-font">{{ $users['hospital']->full_name }}</a></h2>
                        <span class="text-gray-600 fs-5">
                            <i class="fa-solid fa-envelope me-1"></i>
                            <a href="mailto:{{ $users['hospital']->email }}"
                               class="text-gray-600 text-decoration-none fs-5 conatent-down">
                ...
                <div class="row justify-content-center">
                    <div class="col-md-4 col-sm-6 col-12 mb-6 mb-md-0">
                        <div class="border rounded-10 p-5 h-100">
                            <h2 class="text-primary mb-3">{{!empty($users['hospital']->cases) ? $users['hospital']->cases->count() : 0}}</h2>
                            <h3 class="fs-5 fw-light text-gray-600 mb-0 conatent-down">{{__('messages.patient.total_cases')}}</h3>
                        </div>
                    </div>
                    <div class="col-md-4 col-sm-6 col-12 mb-6 mb-md-0">
                        <div class="border rounded-10 p-5 h-100">
                            <h2 class="text-primary mb-3">{{!empty($users['hospital']->patients) ? $users['hospital']->patients->count() : 0}}</h2>
                            <h3 class="fs-5 fw-light text-gray-600 mb-0 conatent-down">{{__('messages.patients')}}</h3>
                        </div>
                    </div>
                    <div class="col-md-4 col-sm-6 col-12">
                        <div class="border rounded-10 p-5 h-100">
                            <h2 class="text-primary mb-3">{{!empty($users['hospital']->appointments) ? $users['hospital']->appointments->count() : 0}}</h2>
                            <h3 class="fs-5 fw-light text-gray-600 mb-0 conatent-down">{{__('messages.patient.total_appointments')}}</h3>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="mt-7">
    <ul class="nav nav-tabs mb-5 pb-1 overflow-auto flex-nowrap text-nowrap" id="myTab" role="tablist">
        <li class="nav-item position-relative me-7 mb-3" role="presentation">
            <button class="nav-link active p-0 header-link" id="overview-tab" data-bs-toggle="tab" data-bs-target="#poverview"
                    type="button" role="tab" aria-controls="overview" aria-selected="true">
                {{ __('messages.overview') }}
            </button>
        </li>
        <li class="nav-item position-relative me-7 mb-3" role="presentation">
            <button class="nav-link p-0 header-link" id="cases-tab" data-bs-toggle="tab" data-bs-target="#husers"
                    type="button" role="tab" aria-controls="cases" aria-selected="false">
                {{ __('messages.users') }}
            </button>
        </li>
        <li class="nav-item position-relative me-7 mb-3" role="presentation">
            <button class="nav-link p-0 header-link" id="patients-tab" data-bs-toggle="tab" data-bs-target="#hBillings"
                    type="button" role="tab" aria-controls="patients" aria-selected="false">
                {{ __('messages.billings') }}
            </button>
        </li>
        <li class="nav-item position-relative me-7 mb-3" role="presentation">
            <button class="nav-link p-0 header-link" id="patients-tab" data-bs-toggle="tab" data-bs-target="#hTransaction"
                    type="button" role="tab" aria-controls="patients" aria-selected="false">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                            <label for="name"
                                   class="pb-2 fs-5 text-gray-600 conatent-up">{{ __('messages.hospitals_list.hospital_name').':' }}</label>
                            <span class="fs-5 text-gray-800 conatent-down">{{ $users['hospital']->hospital->hospital_name }}</span>
                        </div>
                        <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                            <label for="name"
                                   class="pb-2 fs-5 text-gray-600 conatent-up">{{ __('messages.user.hospital_slug').':' }}</label>
                            <a href="{{route('front', $users['hospital']->username)}}"
                               class="show-btn text-decoration-none conatent-down"
                               target="_blank">{{$users['hospital']->username}}
                                <span class="fs-5 text-gray-800 text-primary conatent-down"><i
                                            class="fas fa-external-link-alt url-external-link"></i></span>
                            </a>
                        </div>
                        <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                            <label for="name"
                                   class="pb-2 fs-5 text-gray-600 conatent-up">{{ __('messages.user.email').':' }}</label>
                            <span class="fs-5 text-gray-800 conatent-down">{{!empty($users['hospital']->email)?($users['hospital']->email):__('messages.common.n/a')}}</span>
                        </div>
                        <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                            <label for="name"
                                   class="pb-2 fs-5 text-gray-600">{{ __('messages.employee_payroll.role').':' }}</label>
                            <span class="fs-5 text-gray-800 conatent-down">{{!empty($users['hospital']->roles[0]->name)?($users['hospital']->roles[0]->name):__('messages.common.n/a')}}</span>
                        </div>
                        <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                            <label for="name"
                                   class="pb-2 fs-5 text-gray-600 conatent-up">{{ __('messages.user.phone').':' }}</label>
                            <span class="fs-5 text-gray-800 conatent-down">{{!empty($users['hospital']->phone)?($users['hospital']->phone):__('messages.common.n/a')}}</span>
                        </div>
                        <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                            <label for="name"
                                   class="pb-2 fs-5 text-gray-600 conatent-up">{{ __('messages.common.created_at').':' }}</label>
                            <span class="fs-5 text-gray-800 conatent-down">{{ !empty($users['hospital']->created_at) ? $users['hospital']->created_at->diffForHumans() : __('messages.common.n/a') }}</span>
                        </div>
                        <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                            <label for="name"
                                   class="pb-2 fs-5 text-gray-600 conatent-up">{{ __('messages.common.updated_at').':' }}</label>
                            <span class="fs-5 text-gray-800 conatent-down">{{ !empty($users['hospital']->updated_at) ? $users['hospital']->updated_at->diffForHumans() : __('messages.common.n/a') }}</span>
                        </div>
                    </div>
                </div>
            </div>
