<div class="col-xl-4 col-md-6 d-flex align-items-stretch">
    <div class="card pricing-card text-center flex-fill">
        <div class="card-body px-10 py-14">
            <h3 class="text-gray-900 fs-2 heading-bold">{{ $subscriptionsPricingPlan->name }}</h3>
            ...
            @if(count($subscriptionsPricingPlan->planFeatures) > 0)
                <ul class="pricing-plan-features text-gray-600 fs-5 mb-9 subscription-msg">
                    @foreach($subscriptionsPricingPlan->planFeatures as $planFeature)
                    <!-- <i class="fa-solid fa-check me-3"></i> -->
                        <li><img src="/assets1/images/check-item.svg" style="margin-right: 12px;"></i> <span>{{ $planFeature->feature->name }}</span></li>
                        @if($planFeature->feature->name == "SMS / Mail" && ($planFeature->subscriptionPlan->sms_limit) > 0)
                            <li><img src="/assets1/images/check-item.svg" style="margin-right: 12px;"></i>{{ $planFeature->subscriptionPlan->sms_limit }}
                                SMS
                            </li>
                        @endif
                    @endforeach
